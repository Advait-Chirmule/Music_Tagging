...
def extract_features(song_split, rate):
    spec_max = 52.859596
...
@app.route('/MusicPredict.html', methods=['GET', 'POST'])    
def MusicPredict():
    if request.method == 'POST':
        graphback2 = request.files['snippet']

        preds = []
        labels = ["Blues", "Classical", "Country", "Disco", "Hiphop", "Jazz", "Metal", "Pop", "Reggae", "Rock"]

        song, rate = librosa.load(graphback2)
        ...
        model1 = load_model("new_spec_model_spectrogram1.h5")
        model2 = load_model("new_spec_model_spectrogram2.h5")
        model3 = load_model("new_ensemble_mfcc1.h5")
        model4 = load_model("new_ensemble_mfcc2.h5")
        model5 = load_model("new_ensemble_mfcc3.h5")
        model6 = load_model("model_melspectrogram1.h5")

        models = [model1, model2, model3, model4, model5, model6]

        for song_split in song_splits:
            song_stft, song_mel, song_mfcc = extract_features(song_split, rate)
            preds = preds + get_pred(song_stft,song_mel,song_mfcc,models)
        
        final_pred = get_majority(preds)
        label = labels[final_pred[0]]
        graphback = 'The predicted tag is: ' + label
        return render_template('MusicPredict.html', graphback=graphback)
    return render_template('MusicPredict.html')

@app.route('/MusicAboutUs.html')
def MusicAboutUs():
    return render_template('MusicAboutUs.html')
